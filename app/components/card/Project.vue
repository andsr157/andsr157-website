...
const recalcSquares = () => {
  ...
  squareSideLength.value = Math.min(imageContainerWidth, imageContainerHeight) / divisor
...
<template>
  <div class="w-full mb-6 md:mb-8">
    <a :href="props.url">
      <div class="rounded-xl md:rounded-[32px] lg:rounded-[50px] overflow-hidden relative">
        <div ref="imageContainer" class="imageContainer relative z-30 bg-white">
          ...
          >
            <div
              ...
              :style="{ width: `calc(100% / ${gridDivisor})`, height: `calc(100% / ${gridDivisor})` }"
            />
          </div>
        </div>
      </div>
